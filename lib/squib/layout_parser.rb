  ...
  class LayoutParser
    ...
    def self.recurse_extends(yml, key, visited)
      ...
      parent_keys.each do |parent_key|
        from_extends = yml[key].merge(recurse_extends(yml, parent_key, visited)) do |_key, child_val, parent_val|
        ...
        h = h.merge(from_extends) do |_key, _older_sibling, younger_sibling|
    ...
    def self.assert_not_visited(key, visited)
      fail "Invalid layout: circular extends with '#{key}'" if visited.key? key
    end
  end
end
